...
object RunLSA {
  def main(args: Array[String]): Unit = {
    val path = if(args.length > 0) args(0) else "wikidump.xml"   //"hdfs:///user/ds/Wikipedia/"
    ...
    val (docTermMatrix, termIds, docIds, termIdfs) = documentTermMatrix(docTexts, "src/main/resources/stopwords.txt", numTerms)
